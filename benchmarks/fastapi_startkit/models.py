...
_CONNECTIONS = {
    ...
    # explicitly or the connection silently falls back to SQLite. The password is
    # URL-encoded so credentials with special characters don't corrupt the URL.
    "postgres": lambda: PostgresConfig(
        ...
        url=(
            f"postgresql+asyncpg://postgres:{quote_plus(os.environ.get('PASSWORD', ''))}"
    ...
    "mysql": lambda: MySQLConfig(
        ...
        password=os.environ.get("PASSWORD", ""),
        url=(
            f"mysql+aiomysql://root:{quote_plus(os.environ.get('PASSWORD', ''))}"
