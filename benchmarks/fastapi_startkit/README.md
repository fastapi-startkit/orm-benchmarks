...
## Known limitation — MySQL UPDATE

`Update: Whole` (I) and `Update: Partial` (J) do not run on **MySQL**: MasoniteORM's
MySQL grammar table-qualifies UPDATE columns using its default table name `users`
(ignoring the model's `__table__ = "journal"`), producing

```sql
UPDATE `journal` SET `users`.`level` = %s, `users`.`text` = %s WHERE `users`.`id` = %s
```

which MySQL rejects: `(1054, "Unknown column 'users.id' in 'where clause'")`. SQLite,
PostgreSQL, and MySQL `Delete` are unaffected (their grammars don't table-qualify).
